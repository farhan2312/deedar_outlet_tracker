...
import { HEAD_QUARTERS } from "@/features/outlet-tracker/constants";
...
export async function POST(req: Request) {
  ...
  // Admins have no head quarter; SO/ISR must pick one. Area is always optional.
  ...
  if (role !== "admin" && !(HEAD_QUARTERS as readonly string[]).includes(headQuarter)) {
    return NextResponse.json(
      { error: "Please select your head quarter." },
      { status: 400 },
    );
  }
  ...
  await createUser({ name, phone, passwordHash, headQuarter, area, role });
